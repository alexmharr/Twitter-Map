...
alexharr <- getUser("AlexanderHarr")
...
system.time(UW.df <- rbindlist(lapply(UW.followers,as.data.frame)))

length(UW.followers)
ah.follower.IDs <- alexharr$getFollowers(retryOnRateLimit = 180)
length(ah.follower.IDs)
